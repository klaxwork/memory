...
function liTranslit(string) {
    var translit = $.fn.liTranslit({
        string: string
    }).text();
    translit = translit.replace(/[^a-z0-9\-_]+/g, '');
    translit = $.trim(translit.replace(/[_\-]+/g, ' '));
    translit = translit.replace(/[\s]+/g, '-');
    return translit;
}

	...
	<div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
		<div class="panel panel-flat">
			<div class="panel-heading">
				<div class="row">
					<div class="col-md-9">
						<h5 class="panel-title">Товары, загруженные с Hynt.ru <span style="font-size: 0.8em;">(показано товаров: <span class="countProducts">0</span>)</span></h5>
					</div>
					<div class="col-md-3">
						<a class="pull-right" style="position: relative; top: 3px;" href="javascript: void(0);"
						   data-toggle="modal" data-target="#modal_small"> <i class="icon-question3 position-left"></i>
							Помощь</a>
					</div>
				</div>
			</div>
									...
									<div class="form-group">
										<div class="checkbox">
											<label> <input type="checkbox" name="showAll" value="1"
											               style="width: 15px; height: 15px;" class="ShowAll"> Показать
												все </label>
										</div>
									</div>
								</div>
...
<script>
	...
	$(document).ready(function () {
		...
		$.contextMenu({
			...
			items: {
				...
				"EditNomenclature": {
					...
					callback: function (itemKey, opt, rootMenu, originalEvent) {
						window.open(
							'<?= \yii\helpers\Url::to('/cms/product/edit', array('id' => '__ID__')); ?>'
		...
		function copyNoms() {
			...
			var Error = false;
			if (ClipBoard.dstCategoryId == 0) { //(Current.type != 'node') {
				alert('Не выбран конечный узел!');
				Error = true;
			}
			if (ClipBoard.ids.length == 0) { //ClipBoard.type != 'noms') {
				alert('Не выбраны товары! (!noms)');
				Error = true;
			}
			if (0) { //(ClipBoard.copyIds.length == 0) {
			...
			if (!Error) {
				...
				$.ajax({
					type: 'POST',
					dataType: 'json',
					url: '<?php echo \yii\helpers\Url::to('/cms/tree/move-noms'); ?>',
					data: dataForm,
					success: function (response) {
						console.log('[RESPONSE]', response);
						if (response.success) {
							ClipBoard.ids.forEach(function (item, index) {
		...
		function loadNomenclatures() {
			...
			$.ajax({
				type: 'POST',
				dataType: 'json',
				url: '<?= \yii\helpers\Url::to('/cms/product/get-ecm-products', ['id' => '__CATEGORY__']) ?>'.replace('__CATEGORY__', categoryId),
				data: {
				...
				success: function (response) {
					...
					if (response.success) {
						clearNomenclatures();
						$('.countProducts').text(response.countProducts)
			...
			$("#viewtree").fancytree({
				...
				click: function (event, data) {
					...
					if (!data.node.lazy) { // (!data.node.children) {
						console.log('!data.node.lazy');
						console.log('data.node.key', data.node.key);
						categoryId = data.node.key;
						Current.type = 'node';
						Current.id = Number(categoryId);
						ClipBoard.dstCategoryId = Current.id;
						//loadNomenclatures();
						getAll();
						console.log('Current', Current);
						console.log('ClipBoard', ClipBoard);
		...
		function check() {
			...
			if (nomenclatureIds.length > 0 && ClipBoard.dstCategoryId > 0) {

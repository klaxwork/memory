...
//M::printr(\Yii::$app->getRequest()->csrfParam, '\Yii::$app->getRequest()->csrfParam');
//M::printr(\Yii::$app->getRequest()->getCsrfToken(), '\Yii::$app->getRequest()->getCsrfToken()');

//M::printr('prelast.php');
...
<?php if (0) { ?>
    <div class="cms-default-index">
        ...
        <p>
            ...
            controller
            "<?= get_class($this->context) ?>" in the "<?= $this->context->module->id ?>" module.
        </p>
        <p>
...
<?php //= M::printr($oCategory->attributes, '$oCategory->attributes'); ?>
<?= $_filter ?>
<?php
$tm2 = microtime(true);
$r = $tm2 - $tm1;
...
<script>
    ...
    function loadMoreProducts() {
        ...
        $('.preloader').show();
        $.ajax({
            //url: '< ?= Url::to(['/page/filter/loadMoreProducts', 'node_id' => $oCategory->id]) ?>',
            url: '<?= Url::to(['/catalog/default/loadMoreProducts', 'node_id' => $oCategory->id]) ?>',
            type: 'POST',
            dataType: 'json',
            data: {
                'dataFilter': dataFilterSerial,
                'offset': offset
            },
            success: function (response) {
                console.log('[response]', response);
                if (response.success) {
                    ...
                    $('.countAll').text(declOfNum(response.countAll, tovar));
                    $('.Offset').text(Number(response.offset) + Number(response.countProducts));

                    //$('#products').empty();
                    if (response.products != undefined) {
                        $('#products').append(response.products);
                        offset += response.countProducts;
                    }
                    if (!response.more) {
            ...
            error: function (data, key, value) {
                $('.preloader').hide();
                $('#errors').empty().append(printrErrors(response));
                return false;
                //after_send(data);
            }
        });
